...
class CW(Attack):
    ...
    def forward(self, batch):
        ...
        w = self.inverse_tanh_space(batch['img']).detach()
        ...
        best_adv_images = batch['img'].clone().detach()
        best_L2 = 1e10 * torch.ones((len(batch['img']))).to(self.device)
        prev_cost = 1e10
        dim = len(batch['img'].shape)
        ...
        for step in range(self.steps):
            ...
            batch['img'] = self.tanh_space(w)

            # Calculate loss
            # current_L2 = MSELoss(Flatten(adv_images), Flatten(images)).sum(dim=1)
            current_L2 = MSELoss(Flatten(batch['img']), Flatten(images)).sum(dim=1)
            ...
            if self.targeted:
                ...
                import random
                condition = random.choice([0.0, 1.0])
            ...
            best_adv_images = mask * batch['img'].detach() + (1 - mask) * best_adv_images
