...
* ~~`data`（必填）: 指定数据集，支持枚举值:`kitti`, `bdd100k`, `ua-detrac`, `dawn`, `special_vehicle`, `flir_adas`。~~
...
* `scaling_factor`（选填，默认为`0.9`）：缩放因子。
* `interpolate_method`（选填，默认为`bilinear`）：插值方法，支持枚举值（第一个为默认值）:`bilinear`, `nearest`。
* `image_quality`（选填，默认为`90`）：图像质量。
* `filter_kernel_size`（选填，默认为`3`）：滤波器核大小。
