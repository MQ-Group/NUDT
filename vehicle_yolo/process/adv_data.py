...
def adv_train(args):
    ...
    adv_data_fold = f"{args.output_path}/adv_{args.data_name}"
    adv_data_images_fold = f"{adv_data_fold}/images"
    adv_data_labels_fold = f"{adv_data_fold}/labels"
    adv_data_images_train_fold = f"{adv_data_images_fold}/train"
    adv_data_labels_train_fold = f"{adv_data_labels_fold}/train"
    os.makedirs(adv_data_fold, exist_ok=True)
    os.makedirs(adv_data_images_fold, exist_ok=True)
    os.makedirs(adv_data_labels_fold, exist_ok=True)
    os.makedirs(adv_data_images_train_fold, exist_ok=True)
    os.makedirs(adv_data_labels_train_fold, exist_ok=True)
    ...
    for i, batch in enumerate(yolo.test_loader):
        ...
        if (i/total_iamges)*100 <= args.adversarial_sample_proportion: # 对抗样本占比
            adv_images = atk(batch)
        
        # ori_image_save_path = f"{ori_images_flod}/ori_img_{i}_cls_{yolo.data['names'][int(batch['cls'][0].item())]}.jpg"
        # adv_image_save_path = f"{adv_images_flod}/adv_img_{i}_cls_{yolo.data['names'][int(batch['cls'][0].item())]}.jpg"
        # ori_image_save_path = f"{ori_images_flod}/ori_img_{i}_cls_{int(batch['cls'][0].item())}.jpg"
        # adv_image_save_path = f"{adv_images_flod}/adv_img_{i}_cls_{int(batch['cls'][0].item())}.jpg"
        # ori_image_save_path = f"{ori_images_flod}/ori_img_{i}_obj_{batch['cls'].shape[0]}.jpg"
        # adv_image_save_path = f"{adv_images_flod}/adv_img_{i}_obj_{batch['cls'].shape[0]}.jpg"
        
        ori_file_name = f"ori_img_{i}_obj_{batch['cls'].shape[0]}"
        adv_file_name = f"adv_img_{i}_obj_{batch['cls'].shape[0]}"
        ori_image_save_path = f"{adv_data_images_train_fold}/{ori_file_name}.jpg"
        adv_image_save_path = f"{adv_data_images_train_fold}/{adv_file_name}.jpg"
        ori_label_save_path = f"{adv_data_labels_train_fold}/{ori_file_name}.txt"
        adv_label_save_path = f"{adv_data_labels_train_fold}/{adv_file_name}.txt"
        
        to_pil = transforms.ToPILImage()
        pil_image = to_pil(batch['img'][0])
        pil_image.save(ori_image_save_path)
        pil_image = to_pil(adv_images[0])
        pil_image.save(adv_image_save_path)
        
        ori_image_path = batch['im_file'][0]
        ori_label_path = ori_image_path.replace('images', 'labels').split('.')[0] + '.txt'
        os.system(f"cp {ori_label_path} {ori_label_save_path}")
        os.system(f"cp {ori_label_path} {adv_label_save_path}")
    ...
    data = {
        ...
        "attack_method": args.attack_method,
        "original_adversarial_samples_data": adv_data_fold,
        # "original_samples": ori_images_flod,
        # "adversarial_samples_data": adv_images_flod
    }
    sse_print(event, data)
